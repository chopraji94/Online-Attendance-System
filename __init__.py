...
import datetime
import threading
from plotly.offline import plot #Plotly used for plotting the graph of Students attendance count
from plotly.graph_objs import Scatter


app = Flask(__name__)
app.config['SECRET_KEY'] = '8424276c875d016a' # Providin a secret key to app
client = MongoClient("mongodb://test:test123456@ds119663.mlab.com:19663/mydatabase") #Setting Mlab account with file
...
@app.route('/<tname>/', methods=['POST','GET']) #Teacher account to mark the attendence for student
def index(tname):
	if session['username']:
		user = db[tname]
		search = user.find()
...
@app.route('/<tname>/add/', methods=['POST','GET']) #Teacher account to add students
def add(tname):
	if session['username']:
		data = db[tname]
		if request.method == 'POST':
...
@app.route('/student/<sname>/', methods=['POST','GET']) #Student account
def student(sname):
	if session['username']:
		l=[]
		collection = db.teacher
...
@app.route('/<tname>/date/', methods=['POST', 'GET']) #To update the record on a specific date
def date(tname):
	if session['username']:
		new = db[tname]
		search = new.find()
...
@app.route('/<tname>/student', methods=['POST','GET']) #Data Used for plotting the graph
def h(tname):
	user = db[tname]
	if request.method == 'POST':
		search = user.find()
...
@app.route('/logout/') #Logout session manager
def logout():
